...
class train_model:
    ...
    def train(self):
        data =pd.read_csv('./Downloads/training_dataset.csv')
        array = data.values
    ...
    def test(self, test_data):
        try:
            ...
            print("All Factors For Finding Personality has not been  Entered!")
...
def prediction_result(top, aplcnt_name, cv_path, personality_values):
  ...
  #  result.geometry('700x550')
    ...
    result.configure(background='blue')#here
    result.title("Predicted Personality Table")
    
    #Title
    titleFont = font.Font(family='Calibri', size=40, weight='bold')
    Label(result, text="Result - Personality Prediction", foreground='blue', bg='green', font=titleFont, pady=10, anchor=CENTER).pack(fill=BOTH)
    
    Label(result, text = str('{} : {}'.format("Name:", aplcnt_name.get())).title(), foreground='white', bg='black', anchor='w').pack(fill=BOTH)
    Label(result, text = str('{} : {}'.format("Age:", age)), foreground='white', bg='black', anchor='w').pack(fill=BOTH)
    for key in data.keys():
        if data[key] is not None:
...
def perdict_person():
    ...
    top.configure(background='#d9e6ff')
    top.title("Apply For A Job")
    
    #Title
    titleFont = font.Font(family='Aerial', size=20, weight='bold')
    lab=Label(top, text="Personality Predictor", foreground='#3b3b3b', bg='#d9e6ff', font=titleFont, pady=10).pack()

    #Job_Form
    job_list=('Select Job', '101-Developer at TTC', '102-Professor at MIT', '103-Chef at Taj')
    job = StringVar(top)
    job.set(job_list[0])

    l1=Label(top, text="Name of the Applicant", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=130)
    l2=Label(top, text="Age of the Applicant", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=160)
    l3=Label(top, text="Gender", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=190)
    l4=Label(top, text="Resume-Upload", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=220)
    l5=Label(top, text="Openness", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=250)
    l6=Label(top, text="Oraganize", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=280)
    l7=Label(top, text="Conscientiousness", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=310)
    l8=Label(top, text="Experience", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=340)
    l9=Label(top, text="Extraversion", foreground='#3b3b3b', bg='#d9e6ff').place(x=70, y=370)
    ...
    submitBtn=Button(top, padx=2, pady=0, text="Submit", bd=0, foreground='blue', bg='white', font=(12))
...
if __name__ == "__main__":
    ...
    b2=Button(root, padx=4, pady=4, width=30, text="Predict Personality", bg='#3b3b3b', foreground='white', bd=1, font=homeBtnFont, command=perdict_person).place(relx=0.5, rely=0.5, anchor=CENTER)
    root.mainloop()
